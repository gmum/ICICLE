...
class RandomExemplarsSelector(ExemplarsSelector):
    ...
    def _select_indices(self, model: LLL_Net, sel_loader: DataLoader, exemplars_per_class: int, transform) -> Iterable:
        ...
        last_ds = None
        if isinstance(model, LLL_Net_PPNet):
            if hasattr(sel_loader.dataset, 'images'):  # BaseDataset, MemoryDataset
                df = pd.DataFrame(sel_loader.dataset.images, columns=['path'])
                last_ds = sel_loader.dataset
            elif isinstance(sel_loader.dataset, ConcatDataset):
                to_df = []
                for ds in sel_loader.dataset.datasets:
                    if isinstance(ds, BaseDataset):
                        to_df.extend(ds.images)
                        last_ds = ds
                df = pd.DataFrame(to_df, columns=['path'])
            df['core_name'] = df.path.apply(lambda x: x.split('/')[-1].split('.jpg')[0])
        for curr_cls in range(num_cls):
            ...
            if isinstance(model, LLL_Net_PPNet) and (curr_cls in last_ds.labels):
                df_cls = df.iloc[cls_ind]
                names = df_cls['core_name'].unique()
                to_take = np.random.choice(names, exemplars_per_class)
                idx_to_take = df_cls[df_cls['core_name'].isin(to_take)].index.to_list()
                result.extend(idx_to_take)
            else:
