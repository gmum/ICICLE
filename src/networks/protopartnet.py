...
class PPNet_head(nn.Module):
    def __init__(self, prototype_shape, num_prototypes, num_classes, prototype_activation_function, focal,
        ...
        super(PPNet_head, self).__init__()
    ...
    def forward(self, conv_features):
        ...
        min_similarities = F.max_pool2d(similarities,
                                        kernel_size=(distances.size()[2],
                                                     distances.size()[3]))
        min_similarities = min_similarities.view(-1, self.num_prototypes)  # [bs, p]

        if self.focal:
            prototype_activations = self.focal_similarity(distances, min_similarities)
        else:
            prototype_activations = min_similarities
        logits = self.last_layer(prototype_activations)
        return distances, logits, min_distances, self.prototype_class_identity
...
class PPNet(nn.Module):
    ...
    def __init__(self, features, img_size, prototype_shape,
        ...
        self.ppnet_head = PPNet_head(self.prototype_shape, self.num_prototypes, self.num_classes,
        ...
        if batch_min_proto_dist_j < global_min_proto_dist[j]:
            ...
            if model_lll.model.prototype_activation_function == 'log':
                proto_act_img_j = np.log(
                    (proto_dist_img_j + 1) / (proto_dist_img_j + model_lll.model.epsilon))
            elif model_lll.model.prototype_activation_function == 'linear':
                proto_act_img_j = max_dist - proto_dist_img_j
            elif model_lll.model.prototype_activation_function == 'exp':
                proto_act_img_j = np.exp(-proto_dist_img_j)
            else:
